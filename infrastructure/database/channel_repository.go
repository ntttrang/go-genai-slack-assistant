...
func (cr *ChannelRepository) Save(config *domain.ChannelConfig) error {
	languages := ""
...
func (cr *ChannelRepository) GetByChannelID(channelID string) (*domain.ChannelConfig, error) {
	query := `
...
func (cr *ChannelRepository) Update(config *domain.ChannelConfig) error {
	languages := ""
	for i, lang := range config.SourceLanguages {
		if i > 0 {
			languages += ","
...
func (cr *ChannelRepository) GetAll() ([]*domain.ChannelConfig, error) {
	query := `
		SELECT id, channel_id, auto_translate, source_languages, target_language, enabled, created_at, updated_at
		FROM channel_configs
